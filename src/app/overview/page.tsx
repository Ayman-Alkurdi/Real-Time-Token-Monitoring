  ...
  useEffect(() => {
    async function fetchAllSessionsData() {
      try {
        ...
        const sessionInfo: {name: string, createdAt: string}[] = await res.json();
        const sessionNames = sessionInfo.map(s => s.name);
  ...
  return (
    <div className="flex flex-col h-screen">
      <header className="bg-gray-800 text-white p-4 flex justify-between items-center">
        ...
        <div>
          <a href="/overview/daily" className="bg-gray-700 p-2 rounded mr-4">View Daily Overview</a>
